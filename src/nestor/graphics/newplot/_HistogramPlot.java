 ...
 */
class _HistogramPlot extends Plot implements IBAMStatsConstants {
	 ...
	 */
	public _HistogramPlot(IAxis xAxis, IAxis yAxis) {
	...
	private void doPage(Graphics2D g) {
		...
		// Determine number of bar-sets to plot.
		int numberOfPlotLines = _graphModel.getCoordinateCollectionCount();

	

		
		// Work through each bar-set in turn.
		for (int i = 0; i < numberOfPlotLines; i++) {
			
			// Get data for current bar-set.
			coordinateCollection = _graphModel.getCoordinateCollection(i);
			
			// Get the bar-set formatting object for current bar-set data.
	...
	private void drawBar(
		...
		double x1 = trueXOrigin + convert(X, AxisType.X);
		double y1 = trueYOrigin - convert(Y, AxisType.Y);
		...
		// Alter width of bar
		double width = Math.ceil(convert(0.65d, AxisType.X));
		//double width = Math.ceil(convert(1d, AxisType.X)) - 3;
		...
		drawBox(
			g,
			plf,
			(int)Math.ceil(x1 - width/2d),
			(int)y1,
			(int)width,
			(int)trueYOrigin - (int)y1
			);
	...
	private void drawBox(
		...
		// TODO - TOO INFLEXIBLE - need specific histogram renderer
		g.setColor(HISTOGRAM_SHADING_COLOUR);
