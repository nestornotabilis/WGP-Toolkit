...
/**
 * Line plotter widget to be used with PlotPanel.
 * @author  Kevin Ashelford.
 */
public class StatsLinePlotter implements IPlotter, nestor.tools.Constants {
	...
	private final Plot _plot = PlotFactory.createXYPlot();
	...
	public void updatePlot(
		...
		_plot.getXAxis().setMinorTicks(xMinorTicks); 
		_plot.plot();
	...
	public void updatePlot(
		...
		// TODO - Need simpler means of logging data.
		...
		for (int j = 0; j < statsArray.size(); j++) {
			...
			final Stats stats = statsArray.get(j);
			final IHistogram histogram = histogramArray.get(j);
			...
			plfs[i].setLineWidth(2);
			plfs[i].setPlotStyle(PlotStyle.Line);
			plfs[i].setSymbol(Symbol.OpenCircle);
			plfs[i].setSymbolColor(PLOT_COLOURS[colorNumber]);
			plfs[i].setSymbolSize(10);
		...
		float yIncrement = Float.parseFloat(nf.format((float)maxY/10f));
		...
		xAxisFormatter.getMajorGridLineFormatter().setLineColor(Color.lightGray);
		...
		xAxisFormatter.getMinorGridLineFormatter().setLineColor(Color.lightGray);
		...
		xAxisFormatter.getMajorTickLabelFormatter().setDecimalPlaces(1);
		...
		yAxisFormatter.getMajorGridLineFormatter().setLineColor(Color.lightGray);
		...
		yAxisFormatter.getMinorGridLineFormatter().setLineColor(Color.lightGray);
...
*/
	...
	// TODO - NO LONGER USED - to be tidied up
	
	/*
	private ICoordinateCollection getLoggedData(final ICoordinateCollection cc) {

		ArrayList<Coordinate> coordinates = new ArrayList<Coordinate>();
		for (int i = 0; i < cc.getSize(); i++) {
			Coordinate c = cc.getCoordinate(i);
			coordinates.add(
				new Coordinate(Math.log10(c.getX() + LOG_CORRECTION), c.getY())
				);
			}

		return CoordinateCollectionFactory.createCoordinateCollection(
			coordinates,
			cc.getTitle()
			);
